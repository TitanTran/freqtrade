"""SANITY CHECK (not acceptance) for the F4 extension veto.

The 2026-01->07 window is exhausted for tuning (see live-audit memory);
this run only verifies the veto does not catastrophically break the
in-sample edge. Acceptance = forward/dry-run sample only.

Runs veto off/on across the standard three windows:
...
VARIANTS = {
    "veto_off": {"ENABLE_EXTENSION_VETO": False},
    "veto_on":  {"ENABLE_EXTENSION_VETO": True},
}
...
def main() -> None:
    ...
    Path("scratch/ab_extension_veto_results.json").write_text(json.dumps(results, indent=2), encoding="utf-8")
