"""A/B validation for the regime-structure EMA speed
(STRUCT_FAST_EMA_PERIOD / STRUCT_SLOW_EMA_PERIOD).

baseline : EMA50/EMA200 4H  (~33-day memory, current live config)
fast     : EMA20/EMA50 4H   (~8-day memory, catches trend flips days earlier)
chart25_99: EMA25/EMA99 4H  (~16-day memory, matches the trader's chart EMAs)

Runs all three across the same three windows used for the entry-veto matrix:
...
Usage: PYTHONPATH=. python scratch/ab_struct_speed.py
"""
...
VARIANTS = {
    "baseline_ema50_200": {"STRUCT_FAST_EMA_PERIOD": 50, "STRUCT_SLOW_EMA_PERIOD": 200},
    "fast_ema20_50":      {"STRUCT_FAST_EMA_PERIOD": 20, "STRUCT_SLOW_EMA_PERIOD": 50},
    "chart_ema25_99":     {"STRUCT_FAST_EMA_PERIOD": 25, "STRUCT_SLOW_EMA_PERIOD": 99},
}
        ...
        for variant, flags in VARIANTS.items():
            ...
            for window, spec in WINDOWS.items():
                ...
                print(f"    profit {cell['profit_pct']:+.1f}%  PF {cell['pf']}  "
                      ...
                      f"avgWin {cell['avg_win_pct']}%")
    finally:
        STRATEGY_FILE.write_text(original, encoding="utf-8")

    Path("scratch/ab_struct_speed_results.json").write_text(json.dumps(results, indent=2), encoding="utf-8")
    print("\n=== MATRIX (profit% | PF | DD% | trades | deep losers) ===")
    header = f"{'variant':<20}" + "".join(f"{w:<38}" for w in WINDOWS)
    print(header)
